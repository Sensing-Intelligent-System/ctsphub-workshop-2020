...
def rosRGBDCallBack(rgb_data, depth_data):
        ...
        if not draw_contours:
            ...
            if detect_shape:
                ...
                for i in range(contours_len):
                    ...
                    if len(approx) == 3:
                        obj_shape = "triangle"
                        cv2.putText(cv_image, "Triangle", (x, y), font, 1, (0, 255, 255), 2, cv2.LINE_AA)
                    elif len(approx) == 4:
                        obj_shape = "square"
                        cv2.putText(cv_image, "Square", (x, y), font, 1, (0, 255, 255), 2, cv2.LINE_AA)
                    elif len(approx) >= 5:
                        obj_shape = "circle"
                        cv2.putText(cv_image, "Circle", (x, y), font, 1, (0, 255, 255), 2, cv2.LINE_AA)

                    if calculate_size:
                        if obj_shape == "triangle":
                            tri_side_len = get_side_length(approx, cv_depthimage2)
                            if tri_side_len is None:
                                continue
                            tri_area = tri_side_len**2 * math.sqrt(3) / 4
                            string = "%.3e" % tri_area
                            cv2.putText(cv_image, string, (x, y+30), font, 1, (0, 255, 255), 2, cv2.LINE_AA)

                        elif obj_shape == "square":
                            squ_side_len = get_side_length(approx, cv_depthimage2)
                            ...
                            circle_radius = get_radius(approx, cv_depthimage2)
    ...
    img_result_pub.publish(cv_bridge.cv2_to_imgmsg(cv_image, encoding="passthrough"))
...
def get_side_length(cnt_vtx_list, depthimage):
    ...
    for i in range(0, len(cnt_vtx_list)):
        ...
        if not math.isnan(depthimage[int(y)][int(x)]):
            ...
            vtx_3d_temp_list.append(v1)
        else:
            vtx_3d_temp_list.append(None)

    side_len_list = [] #A list which stores side lengths of a polygon

    for i in range(0, len(vtx_3d_temp_list)):
        if vtx_3d_temp_list[i][2] != 0:
            vtx_3d_list.append(vtx_3d_temp_list[i])
    
    # vtx_3d_list.append(vtx_3d_temp_list[0])
    for i in range(0, len(vtx_3d_temp_list)):
        if vtx_3d_temp_list[i][2] != 0:
            vtx_3d_list.append(vtx_3d_temp_list[i])
            break
    ...
    for i in range(0, len(vtx_3d_list) - 1):
        ...
        for j in range(0, len(vector)):
            side_len += vector[j]**2
        side_len_list.append(math.sqrt(side_len))
    ...
    for i in range(0, len(vtx_3d_list)):
        for j in range(0, len(vtx_3d_list)):
            ...
            for k in range(0, len(vector)):
                vector_len = math.sqrt(vector[k]**2)
